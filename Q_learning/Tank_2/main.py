...
def main():
    ...
    for e in range(EPISODES):
        state, action_delay, next_state, episode_reward = (
            environment.reset()
        )  # Reset level in tank
        ...
        if e % MEAN_EPISODE == 0 and e != 0:
            ...
            print(
                "Mean rewards for the last {} of {}/{} episodes : {} explore: {}".format(
                    MEAN_EPISODE,
                    e,
                    EPISODES,
                    mean_reward,
            ...
            if SAVE_ANN_MODEL:
                if mean_reward > max_mean_reward:
                    for i, model in enumerate(agent.ANN_models):
                        model_name = (
                            "/ANN_"
                            + str(NUMBER_OF_HIDDEN_LAYERS)
                            + "HL_"
                            + str(i)
                        )
                        model_path = (
                            "Tank_Q_learning_2/models/saved_models"
                            + model_name
                            + ".h5"
                        )
    ...
    print(
        "Mean rewards for the last 10 episodes: {}".format(
